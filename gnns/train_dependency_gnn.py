...
# -------------------------
# 1) 读取 dependency 数据集 (standard InMemory processed)
# -------------------------
class LoadProcessedDependency(InMemoryDataset):
    """
    读取 output/pyg_radius2_dependency_std/processed 下的 (data, slices)
    """
...
# -------------------------
# 2) 模型：GINEConv 图分类（与 dbg/symbol01 对齐）
# -------------------------
...
# -------------------------
# 4) confusion matrix + macro-F1（与 dbg/symbol01 对齐）
# -------------------------
@torch.no_grad()
...
def main():
    ...
    parser.add_argument("--root", default="output/pyg_radius2_dependency_std", help="dependency dataset root (contains processed/)")
    ...
    ds = LoadProcessedDependency(root=args.root)
