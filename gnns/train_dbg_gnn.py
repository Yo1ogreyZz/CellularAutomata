...
# -------------------------
# 1) 读取你已经生成好的 PyG InMemoryDataset
# -------------------------
class LoadProcessedDBG(InMemoryDataset):
    """
    直接从你生成的 output/pyg_radius2_debruijn_std/processed 读取 (data, slices)
    """
...
# -------------------------
# 4) 指标：confusion matrix + macro-F1
# -------------------------
@torch.no_grad()
...
# -------------------------
# 5) main：训练 + 输出 macro-F1 + confusion matrix
# -------------------------
def main():
    parser = argparse.ArgumentParser()
    parser.add_argument("--root", default="output/pyg_radius2_debruijn_std", help="dataset root (contains processed/)")
    ...
    ds = LoadProcessedDBG(root=args.root)
    ...
    model = DBGClassifier(in_dim=in_dim, edge_dim=edge_dim, hidden=args.hidden, num_classes=num_classes).to(device)
